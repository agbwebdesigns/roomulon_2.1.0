...
async function east(a,b,y)  {
    ...
    if (roomBuilder.keyRoom[number].baddies.length===0)  {
        if (joinedArray.indexOf("East") !== -1)  {  //is East one of the optional directions?
            ...
            textWindowScroller();
            $("#twopointfive").html("");
            if (roomBuilder.keyRoom[number].roomConnectionsArray[2].length===1)  {  //if there is no room created in the direction of east yet
                ...
                return 'success1';
            } else if (roomBuilder.keyRoom[number].roomConnectionsArray[2].length>1)  {  //if there is a room created in this direction already
                let numberWest= roomBuilder.keyRoom[number].roomConnectionsArray[2][1];
